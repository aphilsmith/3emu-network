...
contract('3EMUToken', function(accounts) {
    ...
    it('should allow redeeming and transfer the tokens to us', function() {
        let net = null;
        return 3EMUToken.deployed().then(_net => {
            net = _net;
            return net.redeemTokens("0x2Aacac1412062cAaf70b5a6eB6752Ba6E8E5117f", {
                from: accounts[0],
                gas: 2099999,
                gasPrice: 20000000000
            });
        }).then(ret => {
            assert.ok(getEvent('LogRedeem3EMU', ret), 'should log a Redeem3EMU event');
            return net.balanceOf.call(accounts[0]);
        }).then(balance => {
